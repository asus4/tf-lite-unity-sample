...
namespace TensorFlowLite
{
    ...
    public class PoseSegmentation : System.IDisposable
    {
            ...
            Matrix4x4 cropMatrix, float[,] data, float sigmaColor)
        {
            ...
            if (maskTex == null || maskTex.width != textureSize.x || maskTex.height != textureSize.y)
            {
                DisposeUtil.TryDispose(maskTex);
                maskTex = new RenderTexture(textureSize.x, textureSize.y, 0, RenderTextureFormat.ARGB32)
                {
                    enableRandomWrite = true,
                };
                maskTex.Create();
                compute.SetInt(kCropWidth, textureSize.x);
                compute.SetInt(kCropHeight, textureSize.y);
            }
